# Lisla is a human-readable data serialization language 

Lisla is data serialization language that only supports string and array data types.
...
The root of Lisla is array, so that single `abcd` corresponds to `["abcd"]` of JSON.
...
Lisla supports multi-line quoted string syntax.
...
Lisla also supports escape sequences:
...
Below is the blacklist of whitespace characters. They could cause confusing behaviors. So that, Lisla prohibits the use of them in unquoted strings.
...
Lisla skips unquated empty string.
...
Lisla supports single line comment.
...
Document comment is also described with Lisla. 

### Keeping comment

```
//! Comments start with ! are keeped when Lisla is minified.
...
- liblisla : Official implementation. 
- lislac   : CLI tools based on liblisla.

## To implement new Lisla parser.

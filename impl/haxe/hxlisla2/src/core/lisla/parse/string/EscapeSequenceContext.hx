    ...
    public function process(codePoint:CodePoint, quoted:Bool):EscapeResult
	{
		...
		return switch [code, state]
		{
			...
			case [CodePointTools.CLOSEING_BRACE, EscapeSequenceState.UnicodeBody(count, value)]:
				if (count == 0 || 6 < count)
				{
					top.error(BasicParseErrorKind.InvalidDigitUnicodeEscape, Range.createWithEnd(startPosition, top.position));
					EscapeResult.Letter("");
				}
				else
				{
			...
			case [_, _]:
				top.error(BasicParseErrorKind.InvalidEscapeSequence, Range.createWithEnd(startPosition, top.position));
				EscapeResult.Letter("");
		}
	}
}

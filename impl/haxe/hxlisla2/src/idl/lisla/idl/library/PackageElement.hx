...
class PackageElement
{
	...
	public function resolveModule(context:LoadTypesContext):Void
	{
		...
		var typeMap = switch (getTypeMap(idl.data.typeDefinitions, idl.file))
        {
            ...
            case Result.Error(error):
                module = ModuleState.Resolved(Result.Error([new LoadIdlError(idl.file, error)]), idl.file);
                return;
        }
    ...
    ):Result<Map<String, TypeDefinition>, LoadIdlErrorKind>
	{
		...
		for (type in types)
		{
			...
			{
				var path = new TypePath(Maybe.some(path.toModulePath()), name, name.metadata);
				return Result.Error(LoadIdlErrorKind.TypeNameDuplicated(path));
			}
			else
			{
        ...
        return switch (module)
		{
			case ModuleState.Validated(data, file):
                switch (data.getMaybe(typeName.toString()).toOption())
                {
                    ...
                    case Option.Some(Result.Error(errors)):
                        for (error in errors)
                        {
	...
	public function getTypeDefinition(context:LoadTypesContext, typeName:TypeName):Maybe<TypeDefinition>
	{
        ...
        return switch (module)
		{
			...
			case ModuleState.Resolved(Result.Error(errors), _):
                context.errors.pushAll(errors);
                Maybe.none();
	...
	private function readIdl(context:LoadTypesContext):Maybe<LoadedModule>
	{
	...
	{	
		return switch (module)
		{
			case ModuleState.Resolved(Result.Error(errors), file):
                context.errors.pushAll(errors);
                false;

...
import litll.idl.exception.IdlSourceProviderException;
...
@:access(litll.idl.project.source.IdlSourceProviderImpl)
private class TypeGroupResolver
{
	...
	private function _readModule(currentPackage:PackageElement, currentPath:ModulePath):Void
	{
		...
		for (base in provider.directories)
		{
			...
			var litllArray = switch (Parser.run(content))
			{
				case Result.Err(error):
					addError(filePath, IdlReadErrorKind.Parse(error));
					continue;
					
				case Result.Ok(_litllArray):
					_litllArray;
			}
			...
			var idl = switch (IdlLitllfier.run(litllArray))
			{
				...
				case Result.Err(error):
					addError(filePath, IdlReadErrorKind.Litll(error));
					continue;
			}
		...
		for (base in provider.directories)
		{
			...
			if (FileSystem.exists(dirPath) && FileSystem.isDirectory(dirPath))
			{
				for (childName in FileSystem.readDirectory(dirPath))
				{
					...
					{
						if (StringTools.endsWith(childName, ".idl.litll"))
						{

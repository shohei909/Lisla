...
import sora.core.parse.tag.UnsettledArrayTag;
import sora.core.parse.tag.UnsettledLeadingTag;
import sora.core.parse.tag.UnsettledStringTag;
import sora.core.parse.tag.UnsettledStringTag;
...
using sora.core.char.NewLineCharTools;
using sora.core.char.CodePointTools;
...
class Parser
{
	...
	public static function run(string:String, ?config:ParserConfig):Result<SoraArray, ParseError> 
	{
		if (config == null) 
		{
	...
	// End
	// =============================
	public inline function end():Result<SoraArray, ParseError> 
	{
		position += 1;
	...
	{
		var arr = new SoraArray(output.data, nextOutput.tag.settle(position));
	...
	private inline function endOpennigQuote(singleQuoted:Bool, length:Int):Void
	{
		if (length == 2)
		{
			output.data.push(Sora.Str(new SoraString("", output.popStringTag(position - 2).settle(position))));
	...
	private function endClosedQuotedString(quoteCount:Int, detail:QuotedStringDetail):Void
	{
		...
		var soraString = new SoraString(string, detail.tag.settle(position));
	...
	private function endUnquotedString(detail:UnquotedStringDetail):Void
	{
		...
		var soraString = new SoraString(detail.string, detail.tag.settle(position));
		output.data.push(Sora.Str(soraString));
...
{
	public var data(default, null):Array<Sora>;
	...
	public inline function end(position:Int):SoraArray
	{
		return new SoraArray(data, tag.settle(position));
	}

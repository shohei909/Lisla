...
impl SpaceContext {
    pub fn new(start:usize, separater_required: bool) -> SpaceContext {
        SpaceContext {
            data: SpaceData::new(start, separater_required),
    ...
    pub fn complete(self, input:&EndInput) -> Option<Space> {
        match self.kind {
            SpaceContextKind::Normal(context) => 
                self.data.complete_normal(input.index, context),

            SpaceContextKind::Comment(context) => 
                self.data.complete_comment(input.index, context),
        }
    }
}
...
struct SpaceData {
    start: usize,
    lines: Vec<SpaceLine>,
    separater_required: bool,
}

impl SpaceData {
    pub fn new(start:usize, separater_required:bool) -> SpaceData {
        SpaceData {
            start,
            lines: Vec::new(),
            separater_required, 
    ...
    fn process_normal(
        ...
        match character {
            '(' => {
                let space = self.complete_normal(input.index, normal_context);
            ...
            _ => {
                if self.separater_required {
                    let range = Range::with_length(input.index, 1);
                    errors.push(ParseError::from(SeparaterRequiredError{ range }));
                }
    ...
    fn space_action(mut self, kind: SpaceContextKind) -> ProcessAction {
        self.separater_required = false;
        ProcessAction::Next(
    ...
    fn complete_normal(mut self, index:usize, normal_context:NormalContext) -> Option<Space> {
    ...
    fn complete_comment(mut self, index:usize, comment_context:CommentContext) -> Option<Space> {

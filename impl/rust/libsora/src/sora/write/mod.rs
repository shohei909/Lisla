...
pub fn write(config: &Config, arr: &SoraArray) -> String {
...
impl<'a> Writer<'a> {
    ...
    #[inline]
    fn write_elements(&mut self, output: &mut String, arr: &SoraArray) -> ArrayContext {
        let mut iter = arr.data.iter();
        let mut separation = SeparationContext::Head;
        let mut has_new_line = false;
        let mut next = iter.next();

        while let Option::Some(sora) = next {
            match *sora {
                Sora::Array(ref child_arr) => {
            ...
            next = iter.next();
            let separater_required = match (sora, next) {
                (&Sora::Array(_), _) | (_, Option::Some(&Sora::Array(_))) => false, 
                _ => true,
            };
    ...
    #[inline]
    fn write_string(&mut self, output: &mut String, sora_string: &SoraString) -> bool {
        let mut quote = sora_string.tag.detail.quote;
        ...
        let escaped_tag = &sora_string.tag.detail.escaped;
        ...
        match self.config.string_mode {
            ...
            StringMode::Pretty => {
                ...
                let len = sora_string.data.len();
                if let QuoteKind::Quoted(quote_char, _) = quote {
                    for character in sora_string.data.chars() {
